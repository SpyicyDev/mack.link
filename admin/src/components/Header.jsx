      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-2 py-3 sm:py-6">
          ...
          <div className="flex items-center space-x-1 sm:space-x-4 flex-shrink-0">
            ...
            {user && (
              <nav className="flex items-center space-x-1 sm:space-x-4" role="navigation" aria-label="User menu">
                <div className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300" role="status" aria-live="polite">
                  <img 
                    src={user.avatar_url} 
                    alt={`${user.login}'s profile picture`}
                    className="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex-shrink-0"
                  />
                  <span className="hidden sm:inline truncate max-w-[8rem] sm:max-w-none">Welcome, {user.login}!</span>
                  <span className="sm:hidden truncate max-w-[6rem]">{user.login}</span>
                </div>
                <button
                  onClick={handleLogout}
                  className="flex items-center space-x-1 px-2 sm:px-3 py-1 text-xs sm:text-sm text-gray-600 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 rounded"
