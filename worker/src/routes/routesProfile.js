...
export async function listProfileLinks(env, request) {
  const rows = await dbAll(env, `SELECT id, title, subtitle, url, icon, type, image_url, image_alt, order_index, is_visible, click_count, created_at, updated_at FROM profile_links ORDER BY order_index ASC`);
...
export async function createProfileLink(env, request) {
  const body = await request.json();
  const { title, subtitle = '', url, icon = '', is_visible = 1, type = 'button', image_url = '', image_alt = '' } = body || {};
  ...
  await dbRun(env, `INSERT INTO profile_links (title, subtitle, url, icon, type, image_url, image_alt, order_index, is_visible, click_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`, [title, subtitle, url, icon, type, image_url, image_alt, orderIndex, is_visible ? 1 : 0, now, now]);
  const row = await dbGet(env, `SELECT id, title, subtitle, url, icon, type, image_url, image_alt, order_index, is_visible, click_count, created_at, updated_at FROM profile_links WHERE order_index = ?`, [orderIndex]);
...
export async function updateProfileLink(env, request, id) {
  ...
  const body = await request.json();
  const allowed = ['title','subtitle','url','icon','is_visible','type','image_url','image_alt'];
  const updates = {};
  ...
  const row = await dbGet(env, `SELECT id, title, subtitle, url, icon, type, image_url, image_alt, order_index, is_visible, click_count, created_at, updated_at FROM profile_links WHERE id = ?`, [id]);
  return withCors(env, new Response(JSON.stringify(row), { headers: { 'Content-Type': 'application/json' } }), request);
}
